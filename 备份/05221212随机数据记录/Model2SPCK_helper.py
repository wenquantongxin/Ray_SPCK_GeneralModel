        ...
        for line in file:
            ...
            if match:
                ...
                if key in ["ControlInterval", "ResetPortTimeout"]:
                    value = float(value)
                elif key in ["Maxtimesteps", "isRandomInit", "num_workers", "dim_u", "dim_y"]:
                    value = int(value)
                setattr(config_values, key, value)
    return config_values

# 写入SPCK subvars文件
def SubvarsFilesInit(num_workers,Reset2Zeros = False): 
    results = []
...
def SubvarsAlwaysRandomInit(num_workers,IntervalTIme = 1):
    while True:
        SubvarsFilesInit(num_workers,Reset2Zeros = False)
...
def copy_files(filename, target_dir):
    ...
    # 为每个工作者创建一份复制的文件
    for i in range(1, ReadConfig().num_workers + 1):
        # 在原始文件名和序号之间添加一个下划线
...
def monitor_processes(processes, script_path, CalLogName, ResetPortTimeout = 100):
    while True:
...
class SPCKenv():
    def __init__(self,port_base):
        ...
        self.StateRecordProb = 0.05 # 在一定概率下记录当前回合状态量数据
